import threading

import matplotlib
import matplotlib.pyplot as plt
# matplotlib.use('Agg')
import cv2
import numpy as np

import contours
import curves 

img = np.ones([500, 500, 3])
master_xs = []
master_ys = []
shift_factor_x = 0
shift_factor_angle = 0
shift_factor_y = 0
break_flag = False

def figure_to_array(fig:plt.figure):
    fig.canvas.draw()
    w,h = fig.canvas.get_width_height()
    ret = np.frombuffer(fig.canvas.tostring_argb(), dtype=np.uint8)
    ret.shape = (w, h, 4)

    return ret[:,:,1:4]

def align_data(xs, ys, indices, alignment_points):
    matplotlib.use('Agg')
    global master_xs, master_ys, break_flag, shift_factor_x, shift_factor_y
    
    xs[indices[0]:indices[1]] = np.add(xs[indices[0]:indices[1]], alignment_points[0][0]) 

    master_xs = xs[indices[0]:indices[1]]
    master_ys = ys[indices[0]:indices[1]]

    for i in range(1, len(indices)): 
        if i + 1 < len(indices):
            slice_xs = xs[indices[i]:indices[i+1]]
            slice_ys = ys[indices[i]:indices[i+1]]
        else:
            slice_xs = xs[indices[i]:len(xs)]
            slice_ys = ys[indices[i]:len(ys)]

        slice_xs, slice_ys = curves.sort_data(slice_xs, slice_ys)
        slice_xs = np.add(slice_xs, alignment_points[i][0])

        # Creates a new thread with target function 
        t = threading.Thread(target = display, args=[slice_xs, slice_ys, alignment_points[i]])
        t.daemon = True
        t.start()
        t.join()

        if break_flag == True:
            break
        else:
            if i + 1 < len(indices):
                # xs[indices[i]:indices[i+1]] = np.add(xs[indices[i]:indices[i+1]], -1*alignment_points[i])#shift_factor_x)
                xs[indices[i]:indices[i+1]], ys[indices[i]:indices[i+1]], _, _ = gen_disp_pts(slice_xs, slice_ys, 0, alignment_points[i][1])
            else:
                # xs[indices[i]:len(xs)] = np.add(xs[indices[i]:len(xs)], -1*alignment_points[i])# shift_factor_x)
                xs[indices[i]:len(xs)], ys[indices[i]:len(xs)], _, _ = gen_disp_pts(slice_xs, slice_ys, 0, alignment_points[i][1])

    matplotlib.use('TkAgg')
    return xs, ys

def display(xs, ys, alignment_point):
    global img, break_flag, shift_factor_x, shift_factor_y, shift_factor_angle

    shift_factor_x = 0
    shift_factor_y = 0
    shift_factor_angle = 0

    disp_xs, disp_ys, cx, cy = gen_disp_pts(xs, ys, 0, alignment_point[1])

    img = gen_fig_array(disp_xs, disp_ys, cx, cy)
    cv2.imshow("Image", img)

    while True:
        cv2.imshow("Image", img)
        t = cv2.waitKeyEx(0)
        if t == 110: # n
            cv2.destroyWindow("Image")
            break

        #Fine control 
        elif t == 2424832: #left
            shift_factor_angle += .5
            refresh(xs, ys, alignment_point[0], alignment_point[1])
        elif t == 2490368: #up 
            shift_factor_y += .1
            refresh(xs, ys, alignment_point[0], alignment_point[1])
        elif t == 2555904: #right
            shift_factor_angle -= .5
            refresh(xs, ys, alignment_point[0], alignment_point[1])
        elif t == 2621440: #down
            shift_factor_y -= .1
            refresh(xs, ys, alignment_point[0], alignment_point[1])

        #Coarse control
        elif t == 97: #left
            shift_factor_angle += 5
            refresh(xs, ys, alignment_point[0], alignment_point[1])
        elif t == 119: #up 
            shift_factor_y += 1
            refresh(xs, ys, alignment_point[0], alignment_point[1])
        elif t == 100: #right
            shift_factor_angle -= 5
            refresh(xs, ys, alignment_point[0], alignment_point[1])
        elif t == 115: #down
            shift_factor_y -= 1
            refresh(xs, ys, alignment_point[0], alignment_point[1])
        elif t == 113: # q
            cv2.destroyWindow("Image")
            break_flag = True
            break

def refresh(xs, ys, cx, cy):
    global img

    disp_xs, disp_ys, cx, cy = gen_disp_pts(xs, ys, 0, cy)

    img = gen_fig_array(disp_xs, disp_ys, cx, cy)
    cv2.imshow("Image", img)

def gen_fig_array(xs, ys, cx, cy):
    global master_xs, master_ys

    fig = plt.figure(figsize=(8, 8))
    ax = fig.add_subplot(111)

    ax.scatter(master_xs, master_ys, color='black')
    ax.scatter(xs, ys, color='blue') 
    ax.scatter(cx, cy, color='green') 
    ax.axis([-15, 15, -5, 25])

    fig.tight_layout()
    fig.set_size_inches(10, 10)

    ret = figure_to_array(fig)
    plt.close(fig)
    return ret

def gen_disp_pts(xs, ys, cx, cy):
    global shift_factor_x, shift_factor_y, shift_factor_angle
    disp_xs = np.add(xs, shift_factor_x)
    disp_ys = np.add(ys, shift_factor_y)
    cx += shift_factor_x
    cy += shift_factor_y

    disp_xs, disp_ys = contours.rotate_points(disp_xs, disp_ys, shift_factor_angle, cx, cy)

    return disp_xs, disp_ys, cx, cy